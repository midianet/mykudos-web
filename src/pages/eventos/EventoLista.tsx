...
import { IListagemEvento, EventosService } from '../../shared/services/api/eventos/EventosService';
import { useDebounce } from '../../shared/hooks';
import { Icon, IconButton, LinearProgress, Pagination, Paper, Table, TableBody, TableCell, TableContainer, TableFooter, TableHead, TableRow } from '@mui/material';
import { Environment } from '../../shared/environment';
import { useMessageContext } from '../../shared/contexts';

export const EventoLista: React.FC = () => {
  ...
  const [rows, setRows] = useState<IListagemEvento[]>([]);
  ...
  useEffect(() => {
    setIsLoading(true);
    debounce(() => {
      EventosService.getAll(pagina, busca)
        .then((result) => {
          setIsLoading(false);
  ...
  const onDelete = () => {
    ...
    if(selectedId){
      EventosService.deleteById(selectedId)
        .then(result => {
          if(result instanceof Error){
            showMessage({message: result.message , level:'error'});
          }else{
  ...
  return (
    <LayoutBase titulo="Meus Eventos" toolbar= {
      <BarraAcoesLista
            ...
            {rows.map(row => (
              <TableRow key={row.id}>
                <TableCell>
                  ...
                  <IconButton size="small" onClick={() => navigate(`/eventos/detalhe/${row.id}`)} >
                    <Icon>edit</Icon>
                  </IconButton>
                </TableCell>
